...
# ---------------------------------------------------------------------------
# Negative tests: mismatched bit widths must be rejected at trace time
# ---------------------------------------------------------------------------
MISMATCHED_PAIRS = [
    ("float16", "int32"),    # 16 -> 32
    ("float32", "int16"),    # 32 -> 16
...
@pytest.mark.parametrize("src_dtype,dst_dtype", MISMATCHED_PAIRS)
def test_bitcast_dev_bitwidth_mismatch(src_dtype, dst_dtype):
    """npuir_bitcast asserts src/dst have the same bit width; building the
    kernel must raise AssertionError before we ever get to compilation."""
    with pytest.raises(AssertionError):
        bitcast_kernel(8, 8, src_dtype, dst_dtype)
...
if __name__ == "__main__":
    ...
    for src, dst in MISMATCHED_PAIRS:
        print(f"Running mismatch {src} -> {dst}")
        test_bitcast_dev_bitwidth_mismatch(src, dst)
        print(f"  PASS (raised as expected): {src} -> {dst}")
